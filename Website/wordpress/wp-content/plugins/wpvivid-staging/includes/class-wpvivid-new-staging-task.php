...
class WPvivid_New_Staging_Task
{
    ...
    public function setup_task($options)
    {
        ...
        $this->task['login_url'] = $wpvivid_staging->get_login_url();
    ...
    public function do_finish_staging()
    {
        ...
        $this->access_staging_site();
        $this->repair_htaccess();
        $this->check_wpfastestcache();
        $this->check_hmwp_rule();
    ...
    public function access_staging_site()
    {
        $staging_url=$this->task['new_home_url'];
        $options=array();
        $options['timeout']=30;
        $options['body']=array();
        wp_remote_request($staging_url,$options);
    }

    public function repair_htaccess()
    {
        ...
        if(file_exists($path))
        {
            if(!is_multisite())
            {
                ...
                if( !empty( $matches[1] ) )
                {
                    ...
                    $home_url = '';
                    $new_prefix = $this->task['new_prefix'];
                    $option_name = $new_prefix.'options';
                    $home_url_sql = $db->get_results( $db->prepare( "SELECT * FROM $option_name WHERE option_name = %s", 'home' ) );
    ...
    public function check_wpfastestcache()
    {
        $des_path=$this->task['des_path'];
        $htaccess_file=$des_path.DIRECTORY_SEPARATOR.'.htaccess';
        if(file_exists($htaccess_file))
        {
            ...
            {
                $existing_rules = array_filter(extract_from_markers( $htaccess_file, 'WpFastestCache' ));
                if(!empty($existing_rules))
                {
                    insert_with_markers( $htaccess_file, 'WpFastestCache', '' );
                }

                $existing_rules = array_filter(extract_from_markers( $htaccess_file, 'GzipWpFastestCache' ));
                if(!empty($existing_rules))
                {
                    insert_with_markers( $htaccess_file, 'GzipWpFastestCache', '' );
                }

                $existing_rules = array_filter(extract_from_markers( $htaccess_file, 'LBCWpFastestCache' ));
                if(!empty($existing_rules))
                {
                    insert_with_markers( $htaccess_file, 'LBCWpFastestCache', '' );
                }

                $existing_rules = array_filter(extract_from_markers( $htaccess_file, 'WEBPWpFastestCache' ));
                if(!empty($existing_rules))
                {
                    insert_with_markers( $htaccess_file, 'WEBPWpFastestCache', '' );
                }
            }
            else
            {
                $htaccess = file_get_contents($htaccess_file);
                $pattern     = "/#\s?BEGIN\s?WpFastestCache.*?#\s?END\s?WpFastestCache/s";
                ...
                $pattern     = "/#\s?BEGIN\s?GzipWpFastestCache.*?#\s?END\s?GzipWpFastestCache/s";
                if (preg_match($pattern, $htaccess, $matches)) {
                    if($matches[0] !== '')
                    {
                        $htaccess = preg_replace($pattern, "", $htaccess);
                ...
                $pattern     = "/#\s?BEGIN\s?LBCWpFastestCache.*?#\s?END\s?LBCWpFastestCache/s";
                if (preg_match($pattern, $htaccess, $matches)) {
                    if($matches[0] !== '')
                    {
                        $htaccess = preg_replace($pattern, "", $htaccess);
                ...
                $pattern     = "/#\s?BEGIN\s?WEBPWpFastestCache.*?#\s?END\s?WEBPWpFastestCache/s";
                if (preg_match($pattern, $htaccess, $matches)) {
                    if($matches[0] !== '')
                    {
                        $htaccess = preg_replace($pattern, "", $htaccess);
    ...
    public function check_hmwp_rule()
    {
        $des_path=$this->task['des_path'];
        $htaccess_file=$des_path.DIRECTORY_SEPARATOR.'.htaccess';
        if(file_exists($htaccess_file))
        {
            ...
            if(function_exists('extract_from_markers') && function_exists('insert_with_markers'))
            {
                $existing_rules = array_filter(extract_from_markers( $htaccess_file, 'HMWP_VULNERABILITY' ));
                if(!empty($existing_rules))
                {
                    insert_with_markers( $htaccess_file, 'HMWP_VULNERABILITY', '' );
                }

                $existing_rules = array_filter(extract_from_markers( $htaccess_file, 'HMWP_RULES' ));
                if(!empty($existing_rules))
                {
                    insert_with_markers( $htaccess_file, 'HMWP_RULES', '' );
                }
            }
            else
            {
                $htaccess = file_get_contents($htaccess_file);
                $pattern     = "/#\s?BEGIN\s?HMWP_VULNERABILITY.*?#\s?END\s?HMWP_VULNERABILITY/s";
                if (preg_match($pattern, $htaccess, $matches)) {
                    if($matches[0] !== '')
                    {
                        $htaccess = preg_replace($pattern, "", $htaccess);
                ...
                $pattern     = "/#\s?BEGIN\s?HMWP_RULES.*?#\s?END\s?HMWP_RULES/s";
                if (preg_match($pattern, $htaccess, $matches)) {
                    if($matches[0] !== '')
                    {
                        $htaccess = preg_replace($pattern, "", $htaccess);
    ...
    public function set_staging_site_data()
    {
        ...
        $delete_query = $db->prepare("DELETE FROM {$prefix}options WHERE option_name = %s", 'icl_sitepress_settings');
        //$this->log->WriteLog($delete_query, 'notice');
        if ($db->get_results($delete_query)===false)
        {
            $error=$db->last_error;
        ...
        $delete_query = $db->prepare("DELETE FROM {$prefix}options WHERE option_name = %s", 'WPML(setup)');
        //$this->log->WriteLog($delete_query, 'notice');
        if ($db->get_results($delete_query)===false)
        {
            $error=$db->last_error;
        ...
        $delete_query = $db->prepare("DELETE FROM {$prefix}options WHERE option_name = %s", 'wpvivid_new_remote_list');
        if ($db->get_results($delete_query)===false)
        {
            $error=$db->last_error;
            $this->log->WriteLog($error, 'Warning');
    ...
    public function do_copy_core()
    {
        ...
        {
            if(isset($this->task['setting']['files_permission']))
            {
                if($this->task['setting']['files_permission'] === '0755')
                {
                    @chmod($dec_config,0755);
                }
                else if($this->task['setting']['files_permission'] === '0644')
                {
                    @chmod($dec_config,0644);
                }
                else if($this->task['setting']['files_permission'] === '0775')
                {
                    @chmod($dec_config,0775);
                }
                else
                {
                    @chmod($dec_config,0755);
    ...
    public function do_create_new_wp()
    {
        ...
        update_option('wpvivid_create_freshinstall', true);
    ...
    {
        global $wpdb;
        if($this->task['is_mu_single']&&($table['name']==apply_filters('wpvividstg_get_table_prefix', $wpdb->base_prefix).'users'||$table['name']==apply_filters('wpvividstg_get_table_prefix', $wpdb->base_prefix).'usermeta'))
        {
            $new_table_name=$this->str_replace_first(apply_filters('wpvividstg_get_table_prefix', $wpdb->prefix),$this->task['new_prefix'],$table['name']);
        }
        else
        {
            $new_table_name=$this->str_replace_first(apply_filters('wpvividstg_get_table_prefix', $this->task['old_prefix']),$this->task['new_prefix'],$table['name']);
        }
        ...
        if($this->is_same_database())
        {
            ...
            if($wpdb->query( "INSERT INTO `{$new_table_name}` ".$select )===false)
            {
                ...
                $select =  "SELECT * FROM `{$old_table_name}` {$limit}";
                $this->log->WriteLog("INSERT IGNORE INTO `{$new_table_name}` ".$select,'warning');
                if($wpdb->query( "INSERT IGNORE INTO `{$new_table_name}` ".$select )===false)
                {
                    $error='Failed to insert '.$new_table_name.', error: '.$wpdb->last_error;
    ...
    public function create_table($table_name)
    {
        ...
        $ret['result']='success';
        if($this->task['is_mu_single']&&($table_name==apply_filters('wpvividstg_get_table_prefix', $wpdb->base_prefix).'users'||$table_name==apply_filters('wpvividstg_get_table_prefix', $wpdb->base_prefix).'usermeta'))
        {
            $new_table_name=$this->str_replace_first(apply_filters('wpvividstg_get_table_prefix', $wpdb->prefix),$this->task['new_prefix'],$table_name);
        }
        else
        {
            $new_table_name=$this->str_replace_first(apply_filters('wpvividstg_get_table_prefix', $this->task['old_prefix']),$this->task['new_prefix'],$table_name);
        }
    ...
    public function replace_table_data($table)
    {
        ...
        if(preg_match('/aiowps_audit_log/i', $table['name']) || preg_match('/aiowps_debug_log/i', $table['name']) || preg_match('/itsec_logs/i', $table['name']) ||
            preg_match('/eum_logs/i', $table['name']))
        {
            $this->log->WriteLog('Skip replace table '.$table['name'], 'Warning');
            $table['finished']=1;
            $ret['result']='success';
            $ret['table']=$table;
            return $ret;
    ...
    public function change_wp_temp_config()
    {
        ...
        if( $this->task['db_connect']['use_additional_db'])
        {
            ...
            if (preg_match($pattern, $data, $matches)) {
                $db_name = $matches[0];
                $data = str_replace($db_name, $replacement, $data);
            }
            ...
            if (preg_match($pattern, $data, $matches)) {
                $db_user = $matches[0];
                $data = str_replace($db_user, $replacement, $data);
            }
            ...
            if (preg_match($pattern, $data, $matches)) {
                $db_pass = $matches[0];
                $data = str_replace($db_pass, $replacement, $data);
            }
            ...
            if (preg_match($pattern, $data, $matches)) {
                $db_host = $matches[0];
                $data = str_replace($db_host, $replacement, $data);
            }
        }

        preg_match( "/define\s*\(\s*['\"]WP_ENVIRONMENT_TYPE['\"]\s*,\s*(.*)\s*\);/", $data, $matches );
        if( !empty( $matches[1] ) )
        {
            $this->log->WriteLog('WP_ENVIRONMENT_TYPE found in wp-config.php.vividtemp', 'notice');
            $pattern = "/define\s*\(\s*['\"]WP_ENVIRONMENT_TYPE['\"]\s*,\s*(.*)\s*\);.*/";
            $replace = "define('WP_ENVIRONMENT_TYPE', 'staging'); //";
            $data = preg_replace( array($pattern), $replace, $data );
            if( null === ($data) )
            {
                $this->log->WriteLog('WP_ENVIRONMENT_TYPE not replace in wp-config.php.vividtemp','notice');
                return false;
            }
            else
            {
                $this->log->WriteLog('WP_ENVIRONMENT_TYPE replace in wp-config.php.vividtemp','notice');
            }
        }
        else
        {
            $environment_type = "define('WP_ENVIRONMENT_TYPE', 'staging'); //";
            $pos=strpos($data,'/* That\'s all, stop editing! Happy publishing. */');
            if($pos!==false)
            {
                $string_start=substr($data, 0, $pos);
                $string_end=substr($data, $pos);
                $data=$string_start.PHP_EOL.$environment_type.PHP_EOL.$string_end;
            }
            else
            {
                $pos=strpos($data,'/* That\'s all, stop editing! Happy blogging. */');
                if($pos!==false)
                {
                    $string_start=substr($data, 0, $pos);
                    $string_end=substr($data, $pos);
                ...
                {
                    $php_end_pos=strpos($data,'?>');
                    if($php_end_pos!==false)
                    {
                        $string_start=substr($data, 0, $php_end_pos);
                        $string_end=substr($data, $php_end_pos);
                        $data=$string_start.PHP_EOL.$environment_type.PHP_EOL.$string_end;
                    }
                    else
                    {
                        $data=$data.PHP_EOL.$environment_type;
                    }
                }
            }
            $this->log->WriteLog('WP_ENVIRONMENT_TYPE insert to wp-config.php.vividtemp','notice');
        }
    ...
    public function get_copy_dir_list($key,&$src_path,&$des_path)
    {
        ...
        else if($key=='wp-content')
        {
            ...
            */
            ...
            if(isset($local_setting['path']))
                $exclude_regex[]='#^'.preg_quote($this -> transfer_path($this->get_content_dir($this->task['src_path'])).DIRECTORY_SEPARATOR. $local_setting['path'], '/').'#';

            if(defined('WPVIVID_UPLOADS_ISO_DIR'))
            {
                $exclude_regex[]='#^'.preg_quote($this -> transfer_path($this->get_content_dir($this->task['src_path'])).'/'.WPVIVID_UPLOADS_ISO_DIR, '/').'#';
            }

            $exclude_regex[]='#^'.preg_quote($this -> transfer_path($this->get_content_dir($this->task['src_path'])).DIRECTORY_SEPARATOR.'aiowps_backups', '/').'#';
    ...
    public function set_db_connect_option($additional_db_options)
    {
        if(isset($additional_db_options['additional_database_check']) && $additional_db_options['additional_database_check'] == '1')
        {
            ...
            $this->task['db_connect']['dbuser'] = trim($additional_db_options['additional_database_info']['db_user']);
            $this->task['db_connect']['dbpassword'] = $additional_db_options['additional_database_info']['db_pass'];
            $this->task['db_connect']['dbname'] = trim($additional_db_options['additional_database_info']['db_name']);
            $this->task['db_connect']['dbhost'] = trim($additional_db_options['additional_database_info']['db_host']);
        }
        else
        {
    ...
    public function setup_mu($options)
    {
        ...
        else if($options['is_mu_single'])
        {
            ...
            $this->task['exclude_tables'][] =apply_filters('wpvividstg_get_table_prefix', $wpdb->base_prefix).'site';
            $this->task['exclude_tables'][] =apply_filters('wpvividstg_get_table_prefix', $wpdb->base_prefix).'sitemeta';
            $this->task['exclude_tables'][] =apply_filters('wpvividstg_get_table_prefix', $wpdb->base_prefix).'blogs';
            $this->task['exclude_tables'][] =apply_filters('wpvividstg_get_table_prefix', $wpdb->base_prefix).'blogmeta';
