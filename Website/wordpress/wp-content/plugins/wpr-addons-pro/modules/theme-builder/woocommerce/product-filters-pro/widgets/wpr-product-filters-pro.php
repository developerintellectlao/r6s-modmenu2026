...
class Wpr_Product_Filters_Pro extends Widget_Base {
	...
	public function render_product_taxonomies_filter( $settings ) {
		...
		// Use a more efficient way to count terms related to the current query.
		global $wpdb;
		$terms_array = [];
	
		// Directly query the database to get term counts for the specified taxonomy.
		$query = $wpdb->prepare(
			"SELECT terms.slug, COUNT(*) as count 
