        ...
        if($response!=false)
        {
            ...
            else
            {
                if($retry_count<WPVIVID_ONEDRIVE_RETRY_TIMES)
                {
                    ...
                    $wpvivid_plugin->wpvivid_log->WriteLog('http code is not 200, start retry. http code :'.$http_code.', error: '.json_encode($error),'notice');
                    $ret=$this->get_upload_offset($url);

                    if($ret['result']=='failed')
                    {
                        $http_code = curl_getinfo($curl,CURLINFO_HTTP_CODE);
                        $wpvivid_plugin->wpvivid_log->WriteLog('get_upload_offset failed. http code :'.$http_code.', error: '.json_encode($error),'notice');
                        return $ret;
                    }
