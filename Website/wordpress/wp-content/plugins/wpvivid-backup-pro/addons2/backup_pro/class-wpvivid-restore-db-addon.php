        ...
        if($sub_task['unzip_file']['unzip_finished']==0)
        {
            ...
            if(isset($files[0]['options']['is_crypt'])&&$files[0]['options']['is_crypt']=='1' || $is_crypt === true)
            {
                usort($files, function ($a, $b)
                {
                    ...
                    {
                        $a_remove_suffix = str_replace('.zip','',$a['parent_file']);
                        $b_remove_suffix = str_replace('.zip','',$b['parent_file']);
                        $a_tmp = substr($a_remove_suffix, -3);
                        $b_tmp = substr($b_remove_suffix, -3);
                        if($a_tmp > $b_tmp)
                        {
                            return 1;
                        }
                    ...
                    {
                        $a_remove_suffix = str_replace('.zip','',$a['file_name']);
                        $b_remove_suffix = str_replace('.zip','',$b['file_name']);
                        $a_tmp = substr($a_remove_suffix, -3);
                        $b_tmp = substr($b_remove_suffix, -3);
                        if($a_tmp > $b_tmp)
                        {
